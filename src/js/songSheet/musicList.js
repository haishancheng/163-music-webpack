export default function(){
  let view = {
    ...
    `,
    render(data){
      let {songSheetSongs} = data
      songSheetSongs.map((songSheetSong, index) => {
        let order = (index + 1).toString().length === 2 ? (index + 1) : ('0' + (index + 1))
        let $li = $(
          this.template.replace('{{name}}', songSheetSong.name)
            .replace('{{singer}}', songSheetSong.singer)
            .replace('{{id}}', songSheetSong.id)
            .replace('{{order}}', order)
        )
        $(this.el).append($li)
      })
  ...
  let model = {
    ...
    findSongSheetSongs(songSheetId){
      ...
      return query.find().then((songs) => {
        this.data.songSheetSongs = songs.map((song)=>{
  ...
  let controller = {
    ...
    bindEventHub(){
      window.eventHub.on('songSheetId',(data)=>{
        this.model.data.songSheet = data
        this.model.findSongSheetSongs(this.model.data.songSheet.id).then(()=>{
